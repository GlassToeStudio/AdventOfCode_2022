...
calculate the sum of their total sizes. In the example above, these
...
def get_dir_size(terminal_output):
    fs = defaultdict(int)
    current_dir = []
    for line in terminal_output:
        if line[0] == "$":
            ...
            for i in range(1, len(current_dir)+1):
                fs["/".join(current_dir[:i])] += int(size)
    return fs
...
if __name__ == "__main__":
    with open("Day_07/input.txt", "r", encoding="utf-8") as f:
        data = format_data(f)
        fs = get_dir_size(data)
        print(f"Part 1: {calc_sum(fs)}\nPart 2: {clear_space(fs)}")
