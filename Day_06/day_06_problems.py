...
Your puzzle answer was 1361.The first half of this puzzle is complete! It
...
def format_data(in_file: TextIOWrapper) -> list[str]:
    ...
    return "".join(x.strip() for x in in_file.readlines())


def parse_datastream(data, len_packet=4):
    buffer = deque()
    for i, character in enumerate(data):
        buffer.append(character)
        if len(buffer) == len_packet:
...
if __name__ == "__main__":
    with open("Day_06/input.txt", "r", encoding="utf-8") as f:
        data = format_data(f)
        print(f"Part 1: {parse_datastream(data, 4)}\nPart 2: {parse_datastream(data, 14)}")
