...
def format_data(in_file: TextIOWrapper) -> list[str]:
    ...
    return [tuple(x.split()) for x in in_file.readlines()]
...
def roshambo(data):
    ...
    for him, me in data:
        ...
        match me:
            case "X":  # Rock
                score += check_outcome(him, "A", "C", "B")
            case "Y":  # Paper
                score += check_outcome(him, "B", "A", "C")
            case "Z":  # Scissors
                score += check_outcome(him, "C", "B", "A")
    return score
...
def roshambo_2(data):
    score = 0
    for him, me in data:
        score += SCORES[L_D_W[him][me]]
        match me:
            case "X":  # lose
                score += check_outcome(him, "", "", him)
            case "Y":  # draw
                score += check_outcome(him, him, "", "")
            case "Z":  # win
                score += check_outcome(him, "", him, "")
...
def check_outcome(shape, draw, win, lose):
    if shape == draw:
        return 3
    if shape == win:
        return 6
...
if __name__ == "__main__":
    with open("Day_02/input.txt", 'r', encoding='utf-8') as f:
        data = format_data(f)
        print(f"Part 1: {roshambo(data)}\nPart 2: {roshambo_2(data)}")
