...
Your puzzle answer was 511.The first half of this puzzle is complete! It
...
def format_data(in_file: TextIOWrapper) -> list[str]:
    ...
    return [y.split("-") for x in in_file.readlines() for y in x.strip().split(",")]


def find_complete_overlap(data):
    overlap = 0
    for i in range(0, len(data), 2):
        if int(data[i][0]) <= int(data[i+1][0]) and int(data[i][1]) >= int(data[i+1][1]):
            #print(data[i][0], "(", data[i+1][0], data[i+1][1], ")", data[i][1])
            overlap += 1
            continue
        if int(data[i][0]) >= int(data[i+1][0]) and int(data[i][1]) <= int(data[i+1][1]):
...
def find_any_overlap(data):
    overlap = 0
    for i in range(0, len(data), 2):
        if int(data[i][0]) in range(int(data[i+1][0]), int(data[i+1][1])+1) or int(data[i][1]) in range(int(data[i+1][0]), int(data[i+1][1])+1):
            print(data[i][0], "(", data[i+1][0], data[i+1][1], ")", data[i][1])
            overlap += 1
            continue
        if int(data[i+1][0]) in range(int(data[i][0]), int(data[i][1])+1) or int(data[i+1][1]) in range(int(data[i][0]), int(data[i][1])+1):
            print(data[i][0], "(", data[i+1][0], data[i+1][1], ")", data[i][1])
            overlap += 1
    return overlap
...
if __name__ == "__main__":
    with open("Day_04/input.txt", 'r', encoding='utf-8') as f:
        data = format_data(f)
        print(data)
        print(find_complete_overlap(data))
        print(find_any_overlap(data))
